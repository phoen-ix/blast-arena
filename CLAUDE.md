...
- **TOTP reset**: Admin can reset a user's 2FA via `POST /admin/users/:id/reset-totp` (clears `totp_secret`, `totp_enabled`, `totp_backup_codes`). Users tab shows 2FA status badge and conditional reset button. Audit-logged with action `reset_totp`
