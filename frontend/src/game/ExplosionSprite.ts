  ...
  private createExplosion(explosion: ExplosionState): void {
    ...
    for (let i = 0; i < explosion.cells.length; i++) {
      ...
      if (settings.animations) {
        ...
        // Start small for expansion animation
        sprite.setScale(0.3);
      ...
      if (settings.particles) {
        // Fire particles
        const fireCount = 3 + Math.floor(Math.random() * 2); // 3-4
        const fireEmitter = this.scene.add.particles(x, y, 'particle_fire', {
